		...
		case (CommandProcessing.STEP_START): {
			...
			for (int i = ops.size() - 1; i >= 0; i--) {
				...
				if (ops.get(i).getOffset() < actualFileSize) {
					ops.get(i).setAccessSize(
							actualFileSize - ops.get(i).getOffset());
				} else {
					ops.remove(i);
				}
			}

			if (amountOfDataToRead != cmd.getIOList(rank).getTotalSize()) {
				client.debug("Short read: "
						+ cmd.getIOList(rank).getTotalSize() + " instead of "
						+ amountOfDataToRead
						+ " should be read => file too small \""
						+ actualFileSize + "\"");
			}

			HashMap<Server, ListIO> targetIOServers = cmd.getFile()
					.getDistribution().distributeIOOperation(
							cmd.getIOList(rank), m.getServers());

			/* create an I/O request for each of these servers */
			for (Server server : targetIOServers.keySet()) {
				IGServer sserver = (IGServer) client.getSimulator()
						.getSimulatedComponent(server);
				...
				/* initial job request */
				OUTresults.addNetSend(targetNIC, new RequestRead(iolist, cmd.getFile()), RequestIO.INITIAL_REQUEST_TAG,
						Communicator.IOSERVERS);

				OUTresults.addNetReceive(targetNIC, RequestIO.IO_DATA_TAG,
						Communicator.IOSERVERS);
