...
public class Direct extends CommandImplementation<Filewriteall> {
	...
	public void process(Filewriteall cmd, CommandProcessing OUTresults, GClientProcess client, int step, NetworkJobs compNetJobs) {
		...
		case (CommandProcessing.STEP_START): {
			...
			HashMap<Server, ListIO> targetIOServers = cmd.getFile().getDistribution().distributeIOOperation(cmd.getIOList(rank), m.getServers());
		...
		case (UPDATE_SIZE): {
			...
			for (Integer rank : cmd.getCommunicator().getParticipatingtRanks()) {
				SingleIOOperation op = cmd.getIOList(rank).getIOOperations().get(cmd.getIOList(rank).getIOOperations().size() - 1);
