 ...
 *
 */
public class ReduceScatterFinalMPICH2 extends CommandImplementation<ReduceScatter>{
	final int TAG = 40002;
	...
	public void process(ReduceScatter cmd, ICommandProcessing OUTresults, GClientProcess client, long step, NetworkJobs compNetJobs) {
		...
		if (step == CommandProcessing.STEP_START){			// starting out by eliminating all post po2 ranks
			if(myRank > (1<<highestPowerOfTwo)){			// if i am beyong highest po2 then ...
				int offset = myRank - (1<<highestPowerOfTwo) - 1;
				targetRank = offset * 2;
				OUTresults.addNetSend(targetRank, new NetworkSimpleData(1 + 20), TAG, cmd.getCommunicator());
			}
			OUTresults.setNextStep(++step);
		}else{
			// now its all po2
			final int currentStep = (int) (step - 1);
			if(myRank == 0){							// i am rank 0
				targetRank = 1<<(currentStep);
			}else if(Integer.bitCount(myRank) == 1){	// po2 rank
				if(myRank == 1<<currentStep){			//my turn to receive
					targetRank = myRank - 1 << currentStep;
				}else{									// my turn to send
					targetRank = myRank + 1 << currentStep;
					if(Integer.numberOfLeadingZeros(myRank) < Integer.numberOfLeadingZeros(targetRank)){		// if not done yet
						OUTresults.setNextStep(CommandProcessing.STEP_COMPLETED);
						return;
					}
				}
			}else if(Integer.bitCount(myRank + 1) == 1 && myRank > 2){
				// receive backwards until time comes to send once
				if(myRank > 1<<currentStep){			//my turn to receive
					targetRank = myRank - 1 << currentStep;
				}else{									// my turn to send
					targetRank = myRank + 1 << currentStep;
