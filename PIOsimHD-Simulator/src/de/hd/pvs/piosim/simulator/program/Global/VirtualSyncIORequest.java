 ...
 * @author julian
 */
public class VirtualSyncIORequest
extends CommandImplementation<FileCommand>
{
	static private class CommunicatorCommandWrapper{
		final private FileCommand command;

		public FileCommand getCommand() {
			return command;
		}

		public CommunicatorCommandWrapper(FileCommand cmd) {
			command = cmd;
		}
	...
	private boolean synchronizeClientsWithoutCommunication(ICommandProcessing cmdResults){
		...
		FileCommand cmd = (FileCommand) cmdResults.getInvokingCommand();
...
//			client.debug("Block for " + cmd + " by " + client.getIdentifier() );
	...
	@Override
	public void process(FileCommand cmd,  ICommandProcessing OUTresults, GClientProcess client, long step, NetworkJobs compNetJobs) {
