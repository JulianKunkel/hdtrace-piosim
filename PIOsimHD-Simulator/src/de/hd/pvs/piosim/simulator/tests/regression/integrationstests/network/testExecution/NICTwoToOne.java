...
public class NICTwoToOne extends TestCase implements TestExecution{
			...
			throws Exception
	{
		...
		final MessageMatchingCriterion crit1 = new MessageMatchingCriterion(hostSrc1.getModelComponent(), hostTgt.getModelComponent(), 1, comm, NetworkSimpleData.class);
		final InterProcessNetworkJob job1 = InterProcessNetworkJob.createSendOperation(crit1, new NetworkSimpleData(SIZE), hostTgt);


		final MessageMatchingCriterion crit2 = new MessageMatchingCriterion(hostSrc2.getModelComponent(), hostTgt.getModelComponent(), 1, comm, NetworkSimpleData.class);
		...
		hostTgt.nic.initiateInterProcessReceive(InterProcessNetworkJob.createReceiveOperation(new MessageMatchingCriterion(hostSrc1.getModelComponent(), hostTgt.getModelComponent(), 1, comm, NetworkSimpleData.class), hostTgt), Epoch.ZERO);
	}
}