...
public class TraceFormatFileOpener {
	...
	private void recursivlyCreateTraceSources(TopologyInternalLevel currentTopo, Class<? extends StatisticSource> statCls, Class<? extends TraceSource> traceCls, boolean readNested) throws Exception{		
		...
		// load statistics:
		for(ExternalStatisticsGroup group: projectDescription.getExternalStatisticGroups()){
			...
			if( fileExists(filename) ){
				...
				final StatisticSource statReader = statCls.getConstructor(new Class<?>[]{String.class, ExternalStatisticsGroup.class}).newInstance(new Object[]{filename, group});
