...
/* Must be checked on larger system! */
#define MAX_CPUMASK_THREADS  64

static int
nodeProcessorList(int node, int** list)
{
	FILE *fp; 
    bstring filename;
    ...
    *list = (int*) malloc(MAX_CPUMASK_THREADS * sizeof(int));
	...
	if (NULL != (fp = fopen (bdata(filename), "r"))) 
	{
		bstring src = bread ((bNread) fread, fp);
        struct bstrList* tokens = bsplit(src,(char) ',');
        tmp = bdata(tokens->entry[1]);
        mask = strtoul(tmp, NULL, 16);
    }
    else
    {
        ERROR;
    ...
    for (i=0; i<MAX_CPUMASK_THREADS; i++)
    {
        if (mask&(1UL<<i))
        {
            (*list)[count++] = i;
...
static int
findProcessor(int nodeId, int coreId)
{
    int i;
