...
public class IconManager{	
	...
	/**
	 * Return the original image with its original size
	 * @param icon
	 * @return
	 */
	public ImageIcon getOriginalImage(IconType icon){
		return getInternalIconData(icon, 0).active;
	}

	/**
	 * 
	 * @param icon
	 * @param size if 0 then do no resizing at all.
	 * @return
	 */
	private InternalIcon getInternalIconData(IconType icon, int size){		
		...
		if(cached == null){
			...
			if(size != 0){
				...
				active = new ImageIcon( resizedImg );
			}else{
				active = new ImageIcon( originalImg );
			}
