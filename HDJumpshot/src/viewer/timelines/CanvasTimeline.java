                            ...
                            implements SearchableView, SummarizableView
{
                                    ...
                                    final TimeBoundingBox  timebounds )
    {
        ...
        if ( offImage != null ) {
            ...
            DrawObjects.drawState(offGraphics, coord_xform, (StateTraceEntry) tr.getTraceEntries().get(0) , new ColorAlpha(255,0,0), 1, 1);
            
            DrawObjects.drawState(offGraphics, coord_xform, (StateTraceEntry) tr.getTraceEntries().get(1) , new ColorAlpha(0,255,0), 1, 1);
            
            DrawObjects.drawArrow(offGraphics, coord_xform, new Epoch(4.5), new Epoch(2.0), 1, 1, new ColorAlpha(ColorAlpha.PINK));
            
            for(XMLTraceEntry entry: tr.getTraceEntries()){
            	if(entry.getType() == TYPE.EVENT){                	
            		DrawObjects.drawEvent(offGraphics, coord_xform, (EventTraceEntry) entry, 1, new ColorAlpha(Color.ORANGE));
            	}
            }
    ...
    public Rectangle localRectangleForDrawable(int timeline, int nestingDepth, Epoch startTime, Epoch endTime)
    {
        ...
        rStart      = (float) timeline - nesting_ftr / 2.0f;
        rFinal      = rStart + nesting_ftr;

        yloc   = coord_xform.convertTimelineToPixel( rStart );
        height = coord_xform.convertTimelineToPixel( rFinal ) - yloc;
