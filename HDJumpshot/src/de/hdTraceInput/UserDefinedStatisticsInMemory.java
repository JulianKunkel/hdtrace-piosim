		...
		try{
			while(true){
				...
				if(startedComputation){			
					...
					assert(! Double.isNaN(lastComputedResult));
					assert(! Double.isInfinite(lastComputedResult));
			...
			if (entries.size() == 0){
				System.err.println("WARNING did not find interesting values for user computed value, using constant value");

				final Object [] values = new Object[1];
				values[0] = expression.computeFunction(lastValues, variableNames);
			...
			System.err.println("WARNING invalid expression using 0");
