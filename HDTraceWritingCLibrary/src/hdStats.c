...
/**
 * Write passed timestamp to the start of the group buffer
 */
static int writeGivenTimestampToGroupBuffer(hdStatsGroup *group, uint64_t timestamp);
...
/**
 * Append value at given timestamp to group buffer.
 */
static int appendValueAtTimestampToGroupBuffer(hdStatsGroup *group, void * value_p, hdStatsValueType type, uint64_t timestamp);
 ...
 * @endcode
 * creates a file named @c MyProject_host0_MyGroup.stat
 *
 * @ifnot api_only
 ...
 *  entries are recorded by the group, you have to call \ref hdS_enableGroup.
 *
 * To allow tracing after the actual execution a timestamp needs to be passed
 * to verify the correct order of the timestamps.
 *
 * @ifnot api_only
 * The header part generated by this function is only the closing tag of the
 *  statistics group. For example
        ...
        )
{
	...
	/* write the starttime for the first statistic entry */
	writeGivenTimestampToGroupBuffer(group, timestamp);
	flushGroupBuffer(group);
 ...
 * @ifnot api_only
 * This is a wrapper that cares about the byte order and then calls
 *  \ref appendValueAtTimestampToGroupBuffer.
 * @endif
 *
 * @if api_only
 *  @ingroup hdStats
 ...
 * @param group Statistics Group
 * @param value DOUBLE value to write
 * @param timestamp Timestamp corresponding to the value
 *
 * @return Error state
 *
 * @retval  0 Success
 ...
 * - \ref HDS_ERR_ENTRY_STATE
 * @else
 * - all from \ref appendValueAtTimestampToGroupBuffer
 * @endif
 */
        ...
        )
{
	...
	int ret = appendValueAtTimestampToGroupBuffer(group, &value, DOUBLE, timestamp);

	/* print debug output */
	hd_debug_msg("Group '%s': type=%s value=%f timestamp=%ld",
			group->name, getTypeString(DOUBLE), v, timestamp);
 ...
 *
 * This must be the last hdS_* function called in a program for each
 *  statistics group.
 *
 * @if api_only
 *  @ingroup hdStats
...
/**
 * Write the passed timestamp to the start of the group buffer.
 *
 * The timestamp written is the current number of seconds and nanoseconds
 * since epoch in two 32 bit integers in correct byte order as specified
 * by HDTrace format.
 *
 * @param group  Statistics group to use
 * @param timestamp Passed timestamp to use
 *
 * @return Error state
 *
 * @retval  0  Success
...
static int writeGivenTimestampToGroupBuffer(hdStatsGroup *group, uint64_t timestamp)
{
	...
	/* assure the timestamp will have the correct length */
	assert(sizeof(timestamp) == HDS_TIMESTAMP_LENGTH);

	/* print debug output (before timestamp conversion) */
	hd_debug_msg("Group '%s': timestamp=%ld",
			group->name, timestamp);

	/* convert long timestamp into hdStat known format of 2 int32 values */
	char timestamp_string[20];

	sprintf(timestamp_string, "%llu",(long long int) timestamp);

	timestamp_string[20] = '\0';

	char _sec[11];
	char _nsec[10];

	strncpy(_sec, timestamp_string, sizeof(char) * 10);
	strncpy(_nsec, timestamp_string + 10, sizeof(char) * 9);

	_sec[10] = '\0';
	_nsec[9] = '\0';

	int32_t sec = (int32_t) atoi(_sec);
	int32_t nsec = (int32_t) atoi(_nsec);

	/* assure the timestamp will have the correct length */
	assert(sizeof(sec) + sizeof(nsec) == HDS_TIMESTAMP_LENGTH);

	/* print debug output (after timestamp conversion) */
	hd_debug_msg("Group '%s': timestamp=%10d.%09d",
			group->name, sec, nsec);
...
/**
 * Append value to group buffer. Slightly different to
 * \ref appendValueToGroupBuffer(). Values are added after the actual execution.
 * Therefore timestamps are passed as arguments and not added depending on the
 * current time and date by \ref writeTimestampToGroupBuffer().
 *
 * Byte order of \a *value_p needs to be correct already before calling this
 *  function.
 *
