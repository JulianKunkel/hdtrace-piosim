...
# 86 "src/client/sysint/fs-add.sm"
...
static int compare_hashes(PINT_client_sm *sm_p, job_status_s *js_p)
{
    ...
    assert(js_p->error_code == 0);
    fsid = sm_p->u.get_config.mntent->fs_id;
    count = sm_p->fetch_config.nservers;
    ...
    for (i = 1; i < count; i++)
    {
        if (sm_p->fetch_config.fs_config_buf_size[0] != sm_p->fetch_config.fs_config_buf_size[i])
        {
    ...
    for (i = 0; i < count; i++)
    {
        ret = PINT_util_digest_sha1(sm_p->fetch_config.fs_config_bufs[i], 
