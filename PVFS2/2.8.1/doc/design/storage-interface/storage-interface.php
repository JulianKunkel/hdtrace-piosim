...
<UL>
<LI><B>Multiple storage instances</B>: This interface is intended to
...
<LI><B>Contiguous and noncontiguous data access</B>:  The first cut
...
<I>The interface will not require interface calls to be made in order
for progress to occur.</I>  This implies that threads will be used
underneath where necessary.

<P>
</LI>
<LI><B>Compatibility with flows</B>:  Flows will almost certainly be
...
<LI><B>Consistency semantics</B>:  If we are going to support
...
<LI><B>Error recovery</B>:  The system must detect and report errors
...
<P>
<I>PARTIAL COMPLETION SEMANTICS NEED MUCH WORK!!!</I>

<P>

<H1><A NAME="SECTION00020000000000000000">
2 Storage space concepts</A>
</H1>
...
<P>
Within this storage space are some number of <I>collections</I>, which
...
3 Dataspace concepts</A>
</H1>

<P>
This storage interface stores and accesses what we will call
<I>dataspaces</I>.  These are logical collections of data organized in one
...
a <I>bytestream space</I>, and a dataspace organized by keyword/value
pairs as a keyval dataspace or <I>keyval space</I>.
Each dataspace will have an identifier that is unique to its server,
which we will simply call a <I>handle</I>.  Physically these dataspaces
...
defined four types of <I>system level objects</I>.  These are data files,
...
<P>
Table&nbsp;<A HREF="storage-interface.php#table:storage_errors">1</A> shows values.  All values will be returned as
integers in the native format (size and byte order).

<P>
<I>Needs to be fleshed out.  Need to pick a reasonable prefix.</I>

<P>
<I>Phil:  Once this is fleshed out, can we apply the same sort of
scheme to BMI?  BMI doesn't have a particularly informative error
reporting mechanism.</I> 

<P>
<I>Rob: Definitely.  I would really like to make sure that in
...
<P>
<I>TODO: pick a reasonable prefix for our flags.</I>

<P>

<UL>
<LI><B>FLAG_VTAG</B>:  Indicates that the vtag is valid.
The caller does not have a valid vtag for input, nor does he desire a
valid vtag in response.
</LI>
<LI><B>FLAG_VTAG_RETURN</B>:  Indicates that the caller wishes to obtain
...
5.4 Implementation of keys, values, and hints</A>
</H2>

<P>
<I>TODO: sync. with code on data_sz element.</I>
...
<P>
<I>TODO: we should build hints out of a pair of the TROVE_keyvals.  We'll
call them a TROVE_hint_s in here for now.</I>
...
<P>
<I>Note: need to add valid error values for each function.</I>

<P>
<I>TODO: find a better format for function descriptions.</I>
...
interface does not assign any meaning to the type value. <I>Do we even need
this type field?</I>
...
<I>could this be done with just the hint call?  having an id in this case
is particularly useful ... so we know the operation is completed...</I>
</LI>
</UL>
...
offset and size.  <U>A flag can be passed in if the caller does not
want a vtag returned.</U>  This allows the underlying implementation to avoid the
overhead of calculating the value.

<P>
<I>The size is [in/out] in code?  Figure out semantics!!!</I>

<P>
</LI>
<LI><B>bstream_write_at(
[in]coll_id,
[in]handle,
[in]buffer,
[in]size,
...
<P>
keyval_iterate will always read <I>count</I> items, unless it hits the end
of the keyval space (EOK).  After hitting EOK, <I>count</I> will be set to
the number of pairs processed.  Thus, callers must compare <I>count</I>
...
will the caller know he is at EOK.  The value of <I>position</I> is not
meaningful after reaching EOK.

<P>
</LI>
<LI><B>keyval_iterate_keys(
[in]coll_id,
[in]handle,
[in/out]position, 
[out]key_array,
...
Similar to above, but only returns keys, not corresponding values. <I>need
to fix parameters</I>
</LI>
</UL>
...
5.5.6 Noncontiguous (list) access</A>
</H3>
These functions are used to read noncontiguous byte stream regions or
multiple key/value pairs.

<P>
<I>How do vtags work with noncontiguous calls?</I>
...
<P>
<I>TODO: how to we report partial success for listio calls?</I>

<P>

<UL>
<LI><B>bstream_read_list(
[in]coll_id,
[in]handle,
[in]mem_offset_array,
[in]mem_size_array,
...
5.5.7 Testing for completion</A>
</H3>

<P>
<I>Do we need coll_ids here?</I>
...
<P>
<I>TODO: fix up semantics for testsome; look at MPI functions for ideas.</I>

<P>
<I>wait function for testing purposes if nothing else?</I>
...
<P>
<I>Note: need to discuss completion queue, internal or external?</I>

<P>
<I>Phil: See pvfs2-internal email at 
<BR>
http://beowulf-underground.org/pipermail/pvfs2-internal/2001-October/000010.html
for my thoughts on this topic.</I>

<P>

<H3><A NAME="SECTION00055800000000000000">
5.5.8 Batch operations</A>
</H3>
