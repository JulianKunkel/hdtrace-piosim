...
4.3 Communications model</A>
</H2>

<P>
All communications operations in BMI are nonblocking.  In order to send a
message, the user must first <I>post</I> the message to the interface,
then <I>test</I> it for completion.  The same holds for receiving
...
<A HREF="bmi-design.php#sec:unexp">5.2</A>.
...
<P>
The overall architecture of BMI is shown in Figure <A HREF="bmi-design.php#fig:bmi-arch">1</A>.
Support for individual network protocols is provided by BMI
<I>methods</I>.  There may be any number of methods active at a given
time.  This collection of methods is managed by the <I>method control
layer</I>.  The method control layer is also responsible for presenting the
...
in use.  It does so by maintaining an internal <I>reference list</I> for
...
module must provide (and strictly adhere to) a predefined <I>method
interface</I>.  It supports reliable, ordered delivery and flow control for
...
message size (more on this in section <A HREF="bmi-design.php#sec:short">5.3</A>), host address,
...
BMI provides a special class of messages called <I>unexpected
messages</I>.   This type of message is sent without the receiving host 
...
messages <I>do not</I> indicate that another receive is needed to
...
<I>immediate completion</I> from any post function.  Immediate
...
handled by functions with the <I>_list</I> suffix.
...
6.1 Types and structures</A>
</H2>

<P>

<UL>
<LI><B>Message tags</B>:  Message tags are numerical values that may
...
<LI><B>ID's</B>:  ID's are opaque handles that a caller may use to
...
<LI><B>unexpected_info</B>: This is a struct used to
...
<UL>
<LI><B>BMI_post_send()</B>: Posts a send operation.
</LI>
<LI><B>BMI_post_recv()</B>: Posts a receive operation.
</LI>
<LI><B>BMI_post_sendunexpected()</B>: Posts a send operation
that was not expected by the receiving process.
</LI>
<LI><B>BMI_unpost()</B>: Unposts a previously submitted
operation.  <I>This is a blocking call.</I>
</LI>
<LI><B>BMI_addr_lookup()</B>: Converts the string
...
<UL>
<LI><B>BMI_test()</B>: Tests for completion of a single
operation.
</LI>
<LI><B>BMI_testsome()</B>: Tests for completion of any of a
specified set of operations.
</LI>
<LI><B>BMI_testunexpected()</B>: Tests for arrival of any
unexpected messages.
</LI>
<LI><B>BMI_wait()</B>:  Tests for completion of a single
operation; is allowed to block briefly if no work is available.
</LI>
<LI><B>BMI_waitsome()</B>: Tests for completion of any of a
specified set of operations; is allowed to block briefly if no
work is available.
</LI>
<LI><B>BMI_waitunexpected()</B>: Tests for completion of any
...
<UL>
<LI><B>BMI_memalloc()</B>:  Creates a new buffer. 
</LI>
<LI><B>BMI_memfree()</B>:  Destroys a buffer previously
...
<UL>
<LI><B>BMI_post_send_list()</B>:  Same as BMI_post_send,
except that it allows the caller to specify an array of buffers
and sizes to send from.
</LI>
<LI><B>BMI_post_recv_list()</B>:  Same as BMI_post_recv,
except that it allows the caller to specify an array of buffers
and sizes to receive into.
</LI>
<LI><B>BMI_post_sendunexpected_list()</B>:  Same as
...
<UL>
<LI><B>BMI_initialize()</B>:  Starts the BMI interface; must
be called prior to any other BMI functions.
</LI>
<LI><B>BMI_finalize()</B>:  Shuts down the BMI interface.
</LI>
<LI><B>BMI_set_info()</B>:  Sets optional BMI parameters.
</LI>
<LI><B>BMI_get_info()</B>:  Reads optional BMI parameters.
...
6.2.1 Supported getinfo and setinfo options</A>
</H3>
...
discarded.  <I>NOTE: this option will almost certainly be
deprecated or replaced soon</I>
</LI>
...
<UL>
<LI><I>Return value of API function</I>:  If an API function
...
<LI><I>Operation error code</I>:  This is a value filled in upon
...
7.1 Method interface</A>
</H2>

<P>

<UL>
<LI><B>BMI_method_initialize()</B>:
</LI>
<LI><B>BMI_method_finalize()</B>:
</LI>
<LI><B>BMI_method_post_send()</B>:
</LI>
<LI><B>BMI_method_post_sendunexpected()</B>:
</LI>
<LI><B>BMI_method_post_recv()</B>:
</LI>
<LI><B>BMI_method_unpost()</B>: 
</LI>
<LI><B>BMI_method_addr_lookup()</B>:
</LI>
<LI><B>BMI_method_test()</B>: 
</LI>
<LI><B>BMI_method_testsome()</B>: 
</LI>
<LI><B>BMI_method_testunexpected()</B>:
</LI>
<LI><B>BMI_method_wait()</B>: 
</LI>
<LI><B>BMI_method_waitsome()</B>: 
</LI>
<LI><B>BMI_method_waitunexpected()</B>:
</LI>
<LI><B>BMI_method_memalloc()</B>:
</LI>
<LI><B>BMI_method_memfree()</B>:
</LI>
<LI><B>BMI_method_set_info()</B>:
</LI>
<LI><B>BMI_method_get_info()</B>:
</LI>
<LI><B>BMI_method_post_send_list()</B>:
</LI>
<LI><B>BMI_method_post_sendunexpected_list()</B>:
</LI>
<LI><B>BMI_method_post_recv_list()</B>:
...
<UL>
<LI><B>method_op</B>:  This structure is used to keep track
...
<LI><B>method_addr</B>:  This structure is used to describe
network addresses at the method level.  Like the method_op
structure, it has both generic and private sections.
</LI>
<LI><B>method_unexpected_info</B>: This structure describes
...
<UL>
<LI><B>op_queue_new()</B>: Creates a new operation queue.
</LI>
<LI><B>op_queue_cleanup()</B>:  Destroys an existing
operation queue as well as any operations contained within it.
</LI>
<LI><B>op_queue_add()</B>:  Adds a
method operation onto the tail of a queue.
</LI>
<LI><B>op_queue_remove()</B>:  Removes a specific
operation from the queue in which it resides.
</LI>
<LI><B>op_queue_search()</B>:  Searches for
...
<LI><B>op_queue_empty()</B>: Determines whether a
queue is empty or not.
</LI>
<LI><B>op_queue_count()</B>: Counts the number of
...
<LI><B>op_queue_dump()</B>: Prints out
...
<UL>
<LI><B>alloc_method_op()</B>:  Allocates a new
operation structure.
</LI>
<LI><B>dealloc_method_op()</B>: Deallocates an existing
method operation.
</LI>
</UL>
...
7.3.2 Method address support</A>
</H3>
...
<UL>
<LI><B>alloc_method_addr()</B>:  Creates
a new address structure.  
</LI>
<LI><B>dealloc_method_addr()</B>:  Destroys an
existing method address structure.
</LI>
<LI><B>bmi_method_addr_reg_callback()</B>:
...
7.3.3 Logging and debugging</A>
</H3>

<P>
BMI uses the <I>gossip</I> library for reporting errors and
logging messages.  This mechanism is used in several other
components besides BMI as well.  A discussion of gossip may be
found in the <I>parl-developer-guidelines</I> document.

<P>

<H3><A NAME="SECTION00073400000000000000">
7.3.4 Operation id's</A>
</H3>
...
The <I>id_generator</I> library is available to aid methods in
...
<UL>
<LI><B>id_gen_fast_register()</B>: Registers
a new structure with the interface and creates a new id that may be used
to reference it.
</LI>
<LI><B>id_gen_fast_lookup()</B>:  Returns a pointer to the
...
<UL>
<LI><B>source code</B>: The source code to BMI may be found in the
``pvfs2'' cvs tree, within the pvfs2/src/io/bmi directory.
</LI>
<LI><B>example methods</B>: Two example methods have been created
...
<LI><B>benchmarks</B>:  Benchmarks that compare MPI and BMI
can be found in pvfs2/src/io/bmi/benchmark.   
</LI>
<LI><B>example applications</B>:  Example applications that use BMI
directly may be found in pvfs2/src/io/bmi/examples.   
</LI>
<LI><B>BMI technical paper</B>: work in progress, available in
