...
<LI><A NAME="tex2html56"
  HREF="developer-guidelines.php">1 TODO</A>
...
<LI><A NAME="tex2html84"
  HREF="developer-guidelines.php#SECTION000120000000000000000">About this document ...</A>
</UL>
...
as an alternative to the environment variable used in section <A HREF="#sec:vi_env"><IMG  ALIGN="BOTTOM" BORDER="1" ALT="[*]"
 SRC="file:/usr/share/latex2html/icons/crossref.png"></A>.  The following is an example
...
<P>
<I>Hmm. Should we maybe have a sample project in cvs that anyone can
check out?  Might be nice to be able to play with this stuff without
hurting a real project </I>

<P>

<UL>
<LI><B>export:</B> If you wish to get a copy of a source
...
cvs export -D today &lt;projectname&gt;
 ...
 WIDTH="18" HEIGHT="30" ALIGN="MIDDLE" BORDER="0"
 SRC="img2.png"
 ALT="$&gt;$"> is the name of the project).  When you are done with the
	code you can simply delete the directory.

<P>
</LI>
<LI><B>check out:</B> The check out command is used to obtain a
...
<LI><B>release:</B> When you are done with a project, you may
...
<LI><B>commit:</B> Once you have made any changes to the project,
...
<LI><B>add:</B> The add command is used to add new files to the
...
<LI><B>update:</B> You will often find it necessary to 
...
<LI><B>status:</B> The status command will tell you the status of
...
<P>
(for the sake of clarity, <IMG
 WIDTH="18" HEIGHT="30" ALIGN="MIDDLE" BORDER="0"
 SRC="img1.png"
 ALT="$&lt;$">projectname<IMG
 WIDTH="18" HEIGHT="30" ALIGN="MIDDLE" BORDER="0"
 SRC="img2.png"
 ALT="$&gt;$"> should probably match the name of the directory that
contains the source)  
...
<LI>Always use the -Wstrict-prototypes command line flag to gcc.
	...
	prototypes for all functions.  See section <A HREF="developer-guidelines.php#sec:proto">9.2.1</A> for more
	information.
</LI>
<LI>Make use of the -g option to gcc in development code.  This enables debugging
	symbols for use with gdb (section <A HREF="developer-guidelines.php#sec:gdb">6</A>).  When code is
...
<A HREF="developer-guidelines.php#sec:gcc">5</A>), this just means using the -g option:
...
<UL>
<LI><B>help</B>: Typing just help at the prompt will give you a list
...
<LI><B>break</B>: Break is used for specifying where you would like
...
<LI><B>run</B>:  This command begins execution of your program.  If
...
<LI><B>continue</B>:  Continue causes execution to resume if you are
...
<LI><B>step</B>:  Step lets you execute one line of source code at a
...
<LI><B>next</B>:  Next is very similar to step, except that it
...
<LI><B>print</B>:  The print command is the primary mechanism for
...
struct-<IMG
 WIDTH="18" HEIGHT="30" ALIGN="MIDDLE" BORDER="0"
 SRC="img2.png"
 ALT="$&gt;$">element and *pointer notation.  Parenthesis are often required
in situations where they are not in c notation, however.
</LI>
<LI><B>where</B>:  This command shows your current location in the
...
<LI><B>list</B>:  List is used to list the source code surrounding a
...
7 Makefile tutorial</A>
</H1>

<P>
<I>We may be able to get this from the MPICH coding docs or from
Walt's web page?</I>

<P>

<H1><A NAME="SECTION00090000000000000000">
8 Electric Fence tutorial</A>
</H1>
...
gdb to determine this information (section <A HREF="developer-guidelines.php#sec:gdb">6</A>).

<P>

<H2><A NAME="SECTION00092000000000000000">
8.2 Electric Fence options</A>
</H2>
...
<I>every</I> source code file in the project, include the following text at
...
in the copyright line as well (See section <A HREF="developer-guidelines.php#sec:pvfs-copyright">10.1</A> for information on how do this in PVFS code).  If you wish to credit particular developers or
...
9.1.2 Other source code header information</A>
</H3>
...
function comments described in section <A HREF="developer-guidelines.php#sec:pvfs-comments">10.2</A>.
...
loops, conditional statements, or functions.  For clarity, <I>any</I>
...
9.2 Hints for writing maintainable code</A>
</H2>
...
<LI>If a function will only be called from within the .c file where
	...
	same .c file near the top.  (see section <A HREF="developer-guidelines.php#sec:static">9.2.4</A> for
...
<LI>Put comments describing the behavior of the function just
	...
	<A HREF="developer-guidelines.php#sec:comments">9.1.3</A> for more detailed information about commenting
	functions).
</LI>
<LI>Header files should <I>only</I> contain prototypes and structures
...
9.2.3 Preventing double inclusion</A>
</H3>
...
9.2.5 Initializing variables</A>
</H3>
...
9.2.6 Allocating and deallocating complex structures</A>
</H3>
...
9.2.7 Keeping up with work in progress</A>
</H3>
...
9.2.8 Choosing good variable and function names</A>
</H3>
...
section <A HREF="developer-guidelines.php#sec:pvfs-naming">10.3</A>.

<P>

<H2><A NAME="SECTION000103000000000000000">
9.3 Advanced topics</A>
</H2>

<P>

<H3><A NAME="SECTION000103100000000000000">
9.3.1 Checking for interrupted system calls</A>
</H3>
...
9.3.2 Constant arguments</A>
</H3>

<P>
If you are passing in pointers as arguments to a function, but <I>do
not</I>
...
9.3.3 Obscure coding practices</A>
</H3>
...
9.3.4 Locking data structures</A>
</H3>
...
9.3.9 Typedefs and opaque types</A>
</H3>
...
<UL>
<LI><B>Platform dependence:</B> Different architectures use a
...
<LI><B>Opaque types:</B> Sometimes you wish to have an
...
<P>
<I>Guess I need an example here...</I>
...
<BR>
10.2 Function commenting
</H2>

<P>
<I>Figure out what standard should be to match up with autodocument
tools.  This is on hold until we settle on such a tool, so use standard
put forth in section <A HREF="developer-guidelines.php#sec:comments">9.1.3</A> for now.</I>
...
10.4 Error logging with Gossip</A>
</H2>
...
<UL>
<LI><B>gossip_enable_stderr()</B>:  Directs logging messages to
	stderr.
</LI>
<LI><B>gossip_enable_file(filename, mode)</B>: Directs logging
	messages to a specified file.  The arguments are the same as those
	taken by the <TT>fopen()</TT> function.
</LI>
<LI><B>gossip_enable_syslog(priority)</B>: Directs logging
	to syslog.  The priority argument is the same as that given to the
	<TT>syslog()</TT> function.
</LI>
<LI><B>gossip_set_debug_mask(debug_on, mask)</B>: Turns
	debugging messages on or off and specifies the mask value to use if
	turned on.
</LI>
<LI><B>gossip_disable()</B>: Gracefully shuts down the Gossip
	logging facilities.
</LI>
<LI><B>gossip_debug(mask, format, ...)</B>: Logs a debugging
...
10.5 Suggested error handling</A>
</H2>

<P>

<H3><A NAME="SECTION000115100000000000000">
10.5.1 Traditional application error handling with errno</A>
</H3>
