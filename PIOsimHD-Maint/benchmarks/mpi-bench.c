...
typedef double (*testFunc)(int param1, double * totalTime);
...
static const Test * currentTest = NULL;
...
double reduceTo0(int size, double * totalTime){
  double * sendbuf = mallocWithCheck((long long) size*8);
  double * recvbuf = mallocWithCheck((long long) size*8);
...
double allreduce(int size, double * totalTime){
  double * sendbuf = mallocWithCheck((long long) size*8);
  double * recvbuf = mallocWithCheck((long long) size*8);
...
double bcast(int size, double * totalTime){
  double * buffer = mallocWithCheck((long long) size*8);
...
double barrier(int unused, double * totalTime){
...
double gather(int size, double * totalTime){
  double * sendbuf = mallocWithCheck((long long) size*8);
  double * recvbuf = mallocWithCheck((long long) size*8 * nproc);
...
double allgather(int size, double * totalTime){
  double * sendbuf = mallocWithCheck((long long) size*8);
  double * recvbuf = mallocWithCheck((long long) size*8 * nproc);
...
double scatter(int size, double * totalTime){
  double * sendbuf = mallocWithCheck((long long) size*8 * nproc);
  double * recvbuf = mallocWithCheck((long long) size*8);
...
double sendRecvRightNeighbour(int size, double * totalTime){
  double * sendbuf = mallocWithCheck((long long) size*8);
  double * recvbuf = mallocWithCheck((long long) size*8);
...
double sendRecvPaired(int size, double * totalTime){
...
double sendToRoot(int size, double * totalTime){
  double * sendbuf = mallocWithCheck((long long) size*8);
  double * recvbuf = mallocWithCheck((long long) size*8);
...
double sendRecvToRoot(int size, double * totalTime){
  double * sendbuf = mallocWithCheck((long long) size*8);
  double * recvbuf = mallocWithCheck((long long) size*8);
...
{reduceTo0, "Reduce1000M", 1000llu*1024*1024 / 8},
#endif
...
{allreduce, "Allreduce1000M", 1000llu*1024*1024 / 8},
#endif
...
#ifdef GByte
{bcast, "Broadcast1000M", 1000llu*1024*1024 / 8},
//{bcast, "Broadcast10000M", 10000llu*1024*1024 / 8},
#endif
...
{allgather, "Allgather1000M", 1000llu*1024*1024 / 8},
#endif
...
{gather, "Gather1000M", 1000llu*1024*1024 / 8},
#endif
...
{scatter, "Scatter1000M", 1000llu*1024*1024 / 8},
#endif
...
{sendRecvRightNeighbour, "sendRecvRightNeighbour10000M", 10000llu*1024*1024 / 8},
#endif
...
{sendRecvPaired, "sendRecvPaired10000M", 10000llu*1024*1024 / 8},
#endif
...
//{sendToRoot, "sendToRoot10000M", 10000llu*1024*1024 / 8},
#endif
...
//{sendRecvToRoot, "sendRecvToRoot10000M", 10000llu*1024*1024 / 8},
#endif
{NULL, NULL, 0}
};
